...
AI_SERVICE_URL = "http://localhost:8002" # Updated port
...
def run_test():
    ...
    headers = {"Authorization": token} # Note: Backend currently doesn't check header for this route but good practice
    res = requests.post(f"{API_URL}/classes", json=class_data, headers=headers)
    print(f"Response: {res.status_code} - {res.json()}")

    # Get Class ID (hacky way: list classes)
    ...
    # Get Student ID (Login to get ID)
