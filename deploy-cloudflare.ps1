...
docker-compose -f docker-compose.cloudflare.yml --env-file .env.prod up -d backend ai-service frontend nginx
...
Write-Host "  1. Configure Cloudflare Tunnel to route to http://localhost:80" -ForegroundColor White
...
Write-Host "  - Configure public hostname: labface.site → http://localhost:80" -ForegroundColor White
Write-Host ""
